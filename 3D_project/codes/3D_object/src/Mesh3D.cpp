...
glm::mat3 Mesh3D::getNormalMatrix() {
    update();
    return mtxNormal;
}
...
void Mesh3D::update()
{
    ...
    mtxNormal = mtxRotate;
    mtxNormal=(glm::transpose(glm::inverse(mtxNormal)));
...
void Mesh3D::draw()
{
    if(colorActive||programTexture==0)
    {
        ...
        programColor->setMat3("uMtxNormal",getNormalMatrix());
    ...
    else
    {
        ...
        programTexture->setMat3("uMtxNormal",getNormalMatrix());
